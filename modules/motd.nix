...
let
  ...
  motdScript = pkgs.writeShellScript "dynamic-motd" ''
    ...
    printf "   $(service_status flaresolverr) %-12s" "FlareSolverr"
    printf "   $(service_status podman-profilarr) %-12s\n" "Profilarr"
    ...
    echo -e "   ''${DIM}├─''${NC} FlareSolverr http://$LOCAL_IP:8191"
    echo -e "   ''${DIM}├─''${NC} Profilarr    http://$LOCAL_IP:6868"
    ...
    if [ -n "$TAILSCALE_IP" ]; then
      ...
      echo -e "   ''${DIM}├─''${NC} FlareSolverr http://$TAILSCALE_IP:8191"
      echo -e "   ''${DIM}├─''${NC} Profilarr    http://$TAILSCALE_IP:6868"
